...
fn create_project(title: String, description: String, funding_goal: NumTokens, duration_days: u64) -> Result<String, String> {
    ...
    if funding_goal == candid::Nat::from(0u128) {  // Use `u128` here
    ...
    let project = Project {
        ...
        description,
        funding_goal,
        current_amount: candid::Nat::from(0u128),
        deadline,
        ...
        projects
            ...
            .filter_map(|(id, project)| {
                let total_contribution: NumTokens = project
                    ...
                    .map(|c| c.amount.clone()) // Clone to avoid ownership issues
                    .fold(candid::Nat::from(0u128), |acc, amount| acc + amount);

                if total_contribution > candid::Nat::from(0u128) {  // Use `u128` here
...
async fn contribute(project_id: String, amount: NumTokens) -> Result<BlockIndex, String> {
    if amount == candid::Nat::from(0u128) {  // Use `u128` here
    ...
    let transfer_args = TransferArg {
        ...
        amount: amount.clone(),  // Clone amount here
    ...
    project.current_amount += amount.clone();  // Clone amount here
...
async fn release_funds(project_id: String) -> Result<BlockIndex, String> {
    ...
    // Clone the current_amount before moving it into the transfer_args
    let transfer_amount = project.current_amount.clone();
    ...
    // Call the transfer function (replace this with your actual transfer implementation)
    let result = transfer(transfer_args).await?;
    
    // Update project to mark funds as released
    let mut updated_project = project.clone();
    updated_project.current_amount = candid::Nat::from(0u128); // Funds have been transferred
...
async fn claim_refund(project_id: String) -> Result<BlockIndex, String> {
    ...
    let mut total_contribution: NumTokens = candid::Nat::from(0u128);  // Use `u128` here
    let mut indices_to_remove = Vec::new();

    for (i, contribution) in project.contributors.iter().enumerate() {
        if contribution.contributor == caller {
            total_contribution += contribution.amount.clone();  // Clone amount here
    ...
    let transfer_args = TransferArgs {
        amount: total_contribution.clone(),  // Clone total_contribution here
        to_account: Account {
            owner: caller,
            subaccount: None,
        },
    ...
    updated_project.current_amount -= total_contribution.clone();  // Clone total_contribution here
